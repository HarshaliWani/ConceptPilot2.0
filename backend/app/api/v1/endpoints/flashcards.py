...
def calculate_next_review(
    confidence: int, ease_factor: float, interval: int, repetitions: int
):
    """
...
async def get_topics(
    ...
    return [{"topic": result["_id"], "count": result["count"]} for result in results]
...
async def get_flashcard(
    ...
    try:
        flashcard = await ops.find_one(
            {"_id": ObjectId(flashcard_id), "user_id": user_id}
        )
...
async def review_flashcard(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Failed to review flashcard: {str(e)}"
        )
...
async def update_flashcard(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Failed to update flashcard: {str(e)}"
        )
...
async def delete_flashcard(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Failed to delete flashcard: {str(e)}"
        )
